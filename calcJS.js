const LabelInitialInvestmentAmount = document.querySelector('.form__input--amount'); 
const LabelStakingPercentage = document.querySelector('.form__input--percentage-amount'); 
...
let csvArray = []; 
csvArray.push(new Array('Line #,Reward Date,Investment Amount,Reward Amount,Total Reward Amount To Date,Staking Reward Rate\n'));
let rewardPeriodStartDate = startDate; // initial start date
let rewardPeriodEndDate = getNextRewardDate(rewardPeriodStartDate, paymentDay); 
let totalReward = 0; 
let periodId = 1; 
...
while (true) {
  let daysDifference = calculateDateDiffreneceInDays(rewardPeriodStartDate, rewardPeriodEndDate);

  let periodReward = totalStakingAmount * (Math.pow((1 + stakingPercentage), 1 / 365) - 1) * daysDifference; // formula is taken from https://support.blockchain.com/hc/en-us/articles/5969248325276-How-are-Staking-Rewards-calculated-
  totalReward += periodReward;
  csvArray.push(new Array(periodId, formatDate(rewardPeriodEndDate, "yyyy-MM-dd"), totalStakingAmount.toFixed(6), periodReward.toFixed(6), totalReward.toFixed(6), '7.00%\n'));
...
function getNextRewardDate(inputDate, paymentDay) {
  ...
  let targetRewardDate = new Date(inputDate.getFullYear(), inputDate.getMonth(), paymentDay);
  console.log(inputDate);
  if (inputDate >= targetRewardDate) {
    targetRewardDate = new Date(inputDate.getFullYear(), inputDate.getMonth() + 1, paymentDay);
  }
